...
│   ├── player.js                  # объект player, логика движения, гравитация, коллизии (вертикальные и горизонтальные), autoMove
│   ├── camera.js                  # камера
...
- **Коллизии**: обрабатываются в `updatePlayer()`:
  - **Вертикальные**: проверка приземления на платформы (`prevBottom <= p.y`).
  - **Горизонтальные**: предотвращение застревания в стенах (если игрок не на земле, сдвиг влево/вправо).
- **Ограничения по горизонтали**: игрок не может выйти за левую границу (x < 0). Правая граница уровня (`level.width - player.width`) блокируется только если `autoMove === false`; при `autoMove === true` (после финиша) игрок может уехать за правый край.
...
update() — обновляет фон (sky, mountains) с проверками на существование.
...
Добавлена проверка на наличие world.currentLevel перед обновлением.
