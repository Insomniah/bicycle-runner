...
│   └── styles.css
├── js/
│   ├── main.js                    # игровой цикл, инициализация через world.setLevel, рестарт, debug
...
world.sky = sky; world.mountains = mountains; — регистрация фоновых объектов.
world.setLevel(level1); — установка первого уровня (генерирует уровень, фон, позиционирует игрока).
rocks.generate(); — генерация камней на земле.
addToLayer(...) — регистрация объектов в слоях.
recalcScene(); — пересборка слоя world (добавление платформ).
