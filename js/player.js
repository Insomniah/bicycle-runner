...
export const player = {
  ...
  update(dt) {
    ...
    if (this.x + this.width >= level.width - CONFIG.FINISH_THRESHOLD && window.game.state.gameOver === false) {
    ...
    } else if (window.game.state.gameOver === "complete" && this.autoMove) {
    ...
    if (!this.autoMove && this.x + this.width > level.width && window.game.state.gameOver !== "complete") {
