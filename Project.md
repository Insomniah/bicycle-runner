...
│       └── math.js
├── levels/
│   ├── level1.js                  # данные уровня 1 (платформы, колёса)
│   └── level2.js                  # данные уровня 2
└── assets/
    ├── player/
...
setLevel(level) – переключает уровень, генерирует платформы, колёса (generateWheels), сбрасывает состояние игрока и камеры, перегенерирует фон.
...
update(dt) – обновляет анимацию (накопление времени, переключение кадров).
...
Данные уровней
level1.js и level2.js содержат объекты с полями number, width, height, groundY, platformData[], platforms[], groundPlatforms[], wheelsData[], метод generate() (только для платформ). Колёса создаются централизованно в world.generateWheels.

wheelsData – массив объектов { x, y }, задающих позицию колёс на уровне.

Инициализация игры
В main.js после загрузки страницы:

Вызов loadAllImages() – предзагрузка всех изображений (игрок, горы, камни, колёса).
...
Обновление: window.game.camera.update(), window.game.player.update(dt), window.game.world.update(dt).

Отрисовка: drawLayers(ctx, window.game.camera), window.game.player.draw(ctx, window.game.camera), drawUI(), drawWheelCounter(), window.game.world.drawWheels(ctx, window.game.camera), window.game.drawDebug() (если включён debugMode).

Счётчик колёс: отображается в правом верхнем углу.

Обработка game.state.gameOver: при завершении уровня переключение на следующий с задержкой CONFIG.LEVEL_SWITCH_DELAY.
...
При переключении уровня через setLevel также вызывается rocks.generate() и generateWheels.
